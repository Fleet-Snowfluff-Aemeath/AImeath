...
ch.onMessage((data) => {
  ...
  if (data.type === 'embed') {
    ...
    if (data.action === 'open_plugin') {
      messages.value.push({ text: '🔧 Agent 正在打开 ' + data.plugin + '...', isSelf: false })
      window.parent.postMessage({
        type: 'agent_open_plugin',
        plugin: data.plugin,
        params: data.params || data,
      }, '*')
    } else if (data.action === 'control_plugin') {
      window.parent.postMessage({
        type: 'agent_control_plugin',
        plugin: data.plugin,
        command: data.command || { value: data.value },
      }, '*')
    } else if (data.action === 'close_plugin') {
      window.parent.postMessage({
        type: 'agent_close_plugin',
        plugin: data.plugin,
      }, '*')
    }
    scrollBottom()
