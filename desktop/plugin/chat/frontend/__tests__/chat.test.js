...
describe('Chat', () => {
  ...
  it('agent open_plugin 消息 postMessage 到 parent', async () => {
    const messages = []
    const origPostMessage = window.parent.postMessage
    window.parent.postMessage = (data, origin) => messages.push(data)
    const w = mount(ChatPage)
    fire({ type: 'agent', action: 'open_plugin', plugin: 'snake' })
    await w.vm.$nextTick()
    expect(messages.length).toBe(1)
    expect(messages[0].type).toBe('agent_open_plugin')
    expect(messages[0].plugin).toBe('snake')
    window.parent.postMessage = origPostMessage
  })

  it('agent close_plugin 消息 postMessage 到 parent', async () => {
    const messages = []
    const origPostMessage = window.parent.postMessage
    window.parent.postMessage = (data, origin) => messages.push(data)
    const w = mount(ChatPage)
    fire({ type: 'agent', action: 'close_plugin', plugin: 'pacman' })
    await w.vm.$nextTick()
    expect(messages.length).toBe(1)
    expect(messages[0].type).toBe('agent_close_plugin')
    window.parent.postMessage = origPostMessage
  })
