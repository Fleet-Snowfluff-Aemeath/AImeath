...
function bindSocket() {
  ...
  ch.onOpen((isReconnect) => {
    if (isReconnect) {
      const p = { action: 'resume', plugin: 'terminal' }
  ...
  ch.onMessage((data) => {
    ...
    if (data.type === 'plugin_exited') {
      ch.close()
      term.write('\r\n\x1b[33m[进程已退出，窗口即将关闭]\x1b[0m\r\n')
      setTimeout(() => {
        window.parent.postMessage({ type: 'agent_close_plugin', window_id: data.window_id || WID, plugin: 'terminal' }, '*')
      }, 500)
    }
