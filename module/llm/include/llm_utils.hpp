...
inline boost::json::array get_default_tools()
{
    ...
    t2["function"] = {
        {"name", "control_app"},
        {"description", "向已打开的应用发送操作指令. Direction values: 0=up, 1=down, 2=left, 3=right. For go: -1=pass, -2=resign. For gomoku/snake/go/pacman 棋盘格子类游戏, 使用 coord 传入落子位置坐标 [row,col], 如第一行第一列为 [0,0]. 同名app有多个实例时, 用 instance 参数指定 (从0开始). 状态注入中 #后数字即 instance 值."},
        {"parameters", {
            {"type", "object"},
            {"properties", {
    ...
    t4["function"] = {
        {"name", "get_app_state"},
        {"description", "获取当前已打开 app 的状态信息（棋盘、分数、游戏状态等），用于 agent 感知 app 状态后再决定控制操作. 同名app有多个实例时, 用 instance 参数指定 (从0开始)."},
        {"parameters", {
            {"type", "object"},
            {"properties", {
