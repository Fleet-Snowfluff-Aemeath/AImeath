...
public:
    ...
    void setOutput(app_output_fn cb, void* userdata)
    {
...
private:
    ...
    void  (*set_output_)(void*, app_output_fn, void*) = nullptr;
...
public:
    ...
    AppInstance createInstance(const std::string& config) const
    {
        if (!create_ || !destroy_) return AppInstance{};
        AppInstance inst;
        inst.handle_ = create_(config.c_str());
        if (!inst.handle_) return AppInstance{};
        inst.async_ = async_;
